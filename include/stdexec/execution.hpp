...
namespace stdexec {
  ...
  namespace __compl_sigs {
    ...
    using __for_all_sigs = __minvoke< decltype(__compl_sigs::__for_all_sigs_(
  ...
  struct __concat_completion_signatures<_Completions...> {
    using __t = __minvoke< __mconcat<__munique<__q<completion_signatures>>>, _Completions...>;
  };

  template <class... _Completions>
  using __concat_completion_signatures_t = __t<__concat_completion_signatures<_Completions...>>;

  template <class _Completions, class _Env>
  inline constexpr bool __valid_completion_signatures_ = false;
  template <class... _Sigs, class _Env>
  inline constexpr bool __valid_completion_signatures_<completion_signatures<_Sigs...>, _Env> =
    true;
  template <>
  ...
  template <class _Completions, class _Env>
  concept __valid_completion_signatures = __valid_completion_signatures_<_Completions, _Env>;
  ...
  struct _MISSING_COMPLETION_SIGNAL_<_Tag(_Args...)> {
    ...
    friend auto operator,(_MISSING_COMPLETION_SIGNAL_, auto) -> _MISSING_COMPLETION_SIGNAL_ {
      return {};
    }
  };
  ...
  namespace __receiver_concepts {
    ...
    concept __is_valid_completion = _Completion::template _WITH_RECEIVER_<_Receiver>::value;
  } // namespace __receiver_concepts
  ...
  concept receiver =
    ...
    !!(
      ...
      || tag_invocable<get_env_t, __cref_t<_Receiver>>) &&environment_provider<__cref_t<_Receiver>>
  ...
  concept receiver_of = receiver<_Receiver> && requires(_Completions* __required_completions) {
    {
      __receiver_concepts::__has_completions<remove_cvref_t<_Receiver>>(__required_completions)
  ...
  namespace __get_completion_signatures {
    ...
    using __member_alias_t = typename remove_cvref_t<_Sender>::completion_signatures;
      ...
      static auto __impl() {
        ...
        if constexpr (__with_tag_invoke<_Sender, _Env>) {
          ...
          if constexpr (same_as<_Result, dependent_completion_signatures<no_env>>) {
            ...
            return __mconst< completion_signatures<
  ...
  template <class _Sender>
  concept sender = enable_sender<remove_cvref_t<_Sender>> && environment_provider<__cref_t<_Sender>>
  ...
  concept sender_in = sender<_Sender> && __with_completion_signatures<_Sender, _Env>;
  ...
  concept sender =
    ...
    // in P2300R7.
    __sender<_Sender> && __sender<_Sender, _Env> && environment_provider<__cref_t<_Sender>>
    && move_constructible<remove_cvref_t<_Sender>>
    && constructible_from<remove_cvref_t<_Sender>, _Sender>;
  ...
  using completion_signatures_of_t = __t<__checked_completion_signatures<_Sender, _Env>>;
  ...
  template <                             //
    ...
    requires sender_in<_Sender, _Env>
  using __value_types_of_t = __gather_completions_for<set_value_t, _Sender, _Env, _Tuple, _Variant>;
  ...
  template <class _Sender, class _Env = no_env, template <class...> class _Variant = __variant>
    requires sender_in<_Sender, _Env>
  using error_types_of_t = __error_types_of_t<_Sender, _Env, __q<_Variant>>;

  template <class _Tag, class _Sender, class _Env = no_env>
    requires sender_in<_Sender, _Env>
  using __count_of = __compl_sigs::
    __for_all_sigs< completion_signatures_of_t<_Sender, _Env>, __q<__mfront>, __mcount<_Tag>>;
  ...
  namespace __compl_sigs {
    ...
    template <         //
      ...
      class _SetError, //
      class _SetStopped>
    auto __make(int) -> __compl_sigs_t<_Sender, _Env, _Sigs, _SetValue, _SetError, _SetStopped>;
    ...
    struct connect_t {
      ...
      static constexpr auto __select_impl() noexcept {
        ...
        if constexpr (!enable_sender<remove_cvref_t<_Sender>>)
          __update_sender_type_to_p2300r7_by_adding_enable_sender_trait<remove_cvref_t<_Sender>>();
  ...
  template <class _Sender, class _Receiver>
  concept __nothrow_connectable = noexcept(connect(__declval<_Sender>(), __declval<_Receiver>()));
  ...
  concept sender_to =
    receiver<_Receiver> && sender_in<_Sender, env_of_t<_Receiver>>
    && __receiver_from<_Receiver, _Sender> && requires(_Sender&& __sndr, _Receiver&& __rcvr) {
  ...
  namespace __as_awaitable {
    ...
    using __env_t = __minvoke<__with_default<__q<__call_result_t>, empty_env>, get_env_t, _Promise>;

    template <class _Value>
    struct __receiver_base {
      template <class... _Us>
        requires constructible_from<__value_or_void_t<_Value>, _Us...>
      friend void tag_invoke(set_value_t, __receiver_base&& __self, _Us&&... __us) noexcept try {
  ...
  namespace __start_detached {
    ...
    struct __detached_receiver {
      ...
      struct __t {
        ...
        template <class _Error>
        [[noreturn]] friend void tag_invoke(set_error_t, __t&&, _Error&&) noexcept {
          std::terminate();
        }
    ...
    template <class _Env>
    using __detached_receiver_t = __t<__detached_receiver<__id<remove_cvref_t<_Env>>>>;
  ...
  namespace __execute_ {
    ...
    struct __as_receiver {
      ...
      [[noreturn]] friend void
        tag_invoke(set_error_t, __as_receiver&&, std::exception_ptr) noexcept {
        std::terminate();
      }
...
#define _DEFINE_MEMBER(_TAG) \
  ...
  namespace __split {
    ...
    template <class _Sender, class _Env>
    using __sender_t =
      __t<__sender<stdexec::__cvref_id<_Sender, _Sender>, stdexec::__id<remove_cvref_t<_Env>>>>;
  ...
  namespace __ensure_started {
    ...
    template <class _Sender, class _Env>
    using __sender_t =
      __t<__sender<stdexec::__id<remove_cvref_t<_Sender>>, stdexec::__id<remove_cvref_t<_Env>>>>;

    template <class _Sender, class _Env>
    using __receiver_t =
      __t<__receiver<stdexec::__id<remove_cvref_t<_Sender>>, stdexec::__id<remove_cvref_t<_Env>>>>;
  ...
  namespace __let {
    ...
    using __op_state_for =
      __mcompose< __mbind_back_q<connect_result_t, _Receiver>, __result_sender<_Fun>>;
    ...
    struct __receiver_ {
      ...
      struct __t {
        ...
        friend void tag_invoke(_Tag, __t&& __self, _As&&... __as) noexcept try {
          ...
          auto& __op = __self.__op_state_->__op_state3_.template emplace<__op_state_t>(__conv{[&] {
            return connect(
        ...
        using __operation_base_t =
          stdexec::__t<__operation_base_<_ReceiverId, _Fun, _Let, _Tuples...>>;
    ...
    struct __let_xxx_t {
      ...
      template <class _Sender, class _Fun>
      using __sender =
        stdexec::__t<__let::__sender<stdexec::__id<remove_cvref_t<_Sender>>, _Fun, _LetTag>>;
  ...
  namespace __stopped_as_xxx {
    ...
    struct __sender {
      ...
      struct __t {
        ...
        using __set_value_t = completion_signatures<set_value_t(std::optional<_Tys>...)>;
    ...
    struct stopped_as_optional_t {
      template <sender _Sender>
      auto operator()(_Sender&& __sndr) const -> __t<__sender<stdexec::__id<decay_t<_Sender>>>> {
  ...
  namespace __schedule_from {
    ...
    using __decay_signature =
      __transform< __q<decay_t>, __mcompose<__q<completion_signatures>, __qf<_Tag>>>;
    ...
    struct schedule_from_t {
      ...
      auto operator()(_Scheduler&& __sched, _Sender&& __sndr) const -> stdexec::__t<
        __sender<stdexec::__id<decay_t<_Scheduler>>, stdexec::__id<decay_t<_Sender>>>> {
  ...
  namespace __on {
    ...
    struct on_t {
      ...
      template <scheduler _Scheduler, sender _Sender>
      auto operator()(_Scheduler&& __sched, _Sender&& __sndr) const
        -> __t<__sender<stdexec::__id<decay_t<_Scheduler>>, stdexec::__id<decay_t<_Sender>>>> {
  ...
  namespace __when_all {
    ...
    struct when_all_t {
      ...
      using __sender_t =
        __t<__sender< std::index_sequence_for<_Senders...>, __id<decay_t<_Senders>>...>>;
