...
namespace exec {
  ...
  namespace __async_read_some {
    ...
    struct __async_read_some_t {
      template <scheduler _Scheduler, class... Args>
        requires tag_invocable<__async_read_some_t, _Scheduler, Args...>
      auto operator()(_Scheduler&& __sched, Args&&... args) const
        noexcept(nothrow_tag_invocable<__async_read_some_t, _Scheduler, Args...>)
          -> tag_invoke_result_t<__async_read_some_t, _Scheduler, Args...> {
        return tag_invoke(__async_read_some_t{}, (_Scheduler&&) __sched, (Args&&) args...);
      }
    };
  }

  using __async_read_some::__async_read_some_t;
  inline constexpr __async_read_some_t async_read_some{};

  namespace __async_write_some {
    using namespace stdexec;

    struct __async_write_some_t {
      template <scheduler _Scheduler, class... Args>
        requires tag_invocable<__async_write_some_t, _Scheduler, Args...>
      auto operator()(_Scheduler&& __sched, Args&&... args) const
        noexcept(nothrow_tag_invocable<__async_write_some_t, _Scheduler, Args...>)
          -> tag_invoke_result_t<__async_write_some_t, _Scheduler, Args...> {
        return tag_invoke(__async_write_some_t{}, (_Scheduler&&) __sched, (Args&&) args...);
      }
    };
  }

  using __async_write_some::__async_write_some_t;
  inline constexpr __async_write_some_t async_write_some{};
}